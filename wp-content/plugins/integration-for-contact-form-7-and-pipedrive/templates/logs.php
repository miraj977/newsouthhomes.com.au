...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div>
  <h2  class="vx_img_head"><img alt="<?php _e("Pipedrive Feeds", 'cf7-pipedrive') ?>" title="<?php _e("Pipedrive Feeds", 'cf7-pipedrive') ?>" src="<?php echo $this->get_base_url()?>images/pipedrive-crm-logo.png?ver=1" /> <?php _e("Pipedrive Log", 'cf7-pipedrive'); ?>
  
   <select style="min-width: 120px; max-width: 200px; margin-left: 12px; font-weight: normal;" id="vx_sel_feed">
  <option value=""><?php _e('All Feeds','cf7-pipedrive') ?></option>
  <?php    
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php _e('Search','cf7-pipedrive') ?>" value="<?php echo $this->post('search') ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php _e('All Objects','cf7-pipedrive') ?></option>
  <?php    
  ...
  <option value=""><?php _e('All Status','cf7-pipedrive') ?></option>
  <?php
  ...
  <option value=""><?php _e('All Times','cf7-pipedrive') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php _e('From Date','cf7-pipedrive') ?>From Date" value="<?php if(isset($_REQUEST['start_date'])){echo $this->post('start_date');}?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php if(isset($_REQUEST['end_date'])){echo $this->post('end_date');}?>" placeholder="<?php _e('To Date','cf7-pipedrive') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" title="<?php _e('Search','cf7-pipedrive') ?>" name="search" class="button-secondary button crm_input_inline"><i class="fa fa-search"></i> <?php _e('Search','cf7-pipedrive') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e('Apply','cf7-pipedrive') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e('Apply','cf7-pipedrive') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php _e('Export as CSV','cf7-pipedrive') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php _e('Export as CSV','cf7-pipedrive') ?></button> 
  <?php
  }
  ...
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo   $log_link.'&entry_id='.$entry_id;?>" title="<?php echo sprintf(__('View Entry# %s Logs','cf7-pipedrive'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(__('View Entry# %s Logs','cf7-pipedrive'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $_log_id !=""){
          ?><a href="<?php echo $log_link;?>" title="<?php _e('View All Logs','cf7-pipedrive') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php _e('View All Logs','cf7-pipedrive') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','cf7-pipedrive') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php _e("Pipedrive ID", 'cf7-pipedrive') ?>
  <i class="fa fa-caret-<?php echo $crm_order ?> vx_sort_icon <?php echo $crm_class ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php _e("Entry ID", 'cf7-pipedrive') ?>
  <i class="fa fa-caret-<?php echo $entry_order ?> vx_sort_icon <?php echo $entry_class ?>"></i>                      
  </th>
  <th scope="col"><?php _e('Feed ID', 'cf7-pipedrive') ?> </th>
  <th scope="col" class="manage-column"  data-name="object"><?php _e("Description", 'cf7-pipedrive') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php _e("Time", 'cf7-pipedrive') ?>
  <i class="fa fa-caret-<?php echo $time_order ?> vx_sort_icon <?php echo $time_class ?>"></i>
  </th>
  <th style="width: 40px"><?php _e('Detail','cf7-pipedrive') ?></th>
  </tr>
  </thead>
  ...
  <th><?php _e('Detail','cf7-pipedrive') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo $row['id'] ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo $base_url ?>images/<?php echo $row["status_img"] ?>.png" alt="<?php echo $row["status"] ? __("Active", 'cf7-pipedrive') : __("Inactive", 'cf7-pipedrive');?>" title="<?php echo $row['title'];?>" class="crm_status_img" /></td>
...
$feed_link=$this->get_feed_link($row['feed_id'],$row['form_id']);
 ?><a href="<?php echo $feed_link ?>" title="<?php _e('Edit Feed','cf7-pipedrive') ?>">#<?php echo $row['feed_id'] ?></a><?php  } ?>
 </td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php _e('Expand Details','cf7-pipedrive') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php _e("No Record(s) Found", 'cf7-pipedrive'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php _e('Clear Pipedrive Log','cf7-pipedrive') ?>" href="<?php echo wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce'); ?>"><?php _e('Clear Pipedrive Log','cf7-pipedrive') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php _e('Displaying','cf7-pipedrive') ?> <span id="paging_range_min_header"><?php echo $data['min'] ?></span> - <span id="paging_range_max_header"><?php echo $data['max'] ?></span> of <span id="paging_total_header"><?php echo $data['items'] ?></span></span><?php echo $data['links'] ?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php _e('Please Select Action','cf7-pipedrive') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php _e('Please select at least one entry','cf7-pipedrive') ?>');
    return false;
}
var action=sel.val();
 ...
 if(!confirm('<?php _e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','cf7-pipedrive') ?>')){
  e.preventDefault();    
   ...
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php _e('Pipedrive Logs will be deleted permanently. Do you want to continue?','cf7-pipedrive') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
      next_tr.find('.vxa_entry').slideUp('fast',function(){
       ...
       if(is_main){
    icon.attr('title','<?php _e('Expand Detail','cf7-pipedrive'); ?>');    
    }        
     return;   
    }else{
     next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php _e('Collapse Detail','cf7-pipedrive'); ?>');    
    }    
    }
