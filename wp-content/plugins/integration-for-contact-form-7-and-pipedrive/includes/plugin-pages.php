...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Pipedrive','cf7-pipedrive') ?></label>
  </div>
  <?php  
}
    /**
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'cf7-pipedrive') . '</a>' );
        }
        return $links;
   }
  /**
...
die();
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'cf7-pipedrive'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'cf7-pipedrive'),
   'vx_sel_account' =>__('Select the Pipedrive account you would like to export entries to.', 'cf7-pipedrive'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Pipedrive. Contacts generated by this form will be automatically added to your Pipedrive account.', 'cf7-pipedrive'),
  
  'vx_map_fields' => __('Associate your Pipedrive fields to the appropriate Contact Form fields.', 'cf7-pipedrive'),
  ...
  'vx_manual_export' => __('If you do not want all entries sent to Pipedrive, but only specific, approved entries, check this box. To manually send an entry to Pipedrive, go to Entries, choose the entry you would like to send to Pipedrive, and then click the "Send to Pipedrive" button.', 'cf7-pipedrive'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Pipedrive Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Pipedrive Object selected in the feed.', 'cf7-pipedrive'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'cf7-pipedrive'),
...
<div class="table_head_i"><?php _e('Capsule', 'cf7-pipedrive'); ?></div>  
<div class="vx_group">
...
if(isset($res['error'])){
  ...
  */
  ...
  $(this).text('<?php _e('Show Key','cf7-pipedrive') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','cf7-pipedrive') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','cf7-pipedrive');    
  return;
  }
...
<input type="checkbox" <?php echo $check ?> id="meta_item_<?php echo $k ?>" name="meta[groups][<?php echo $k ?>]" value="1">
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Pipedrive','cf7-pipedrive');
        return $plugins;
    }
  /**
...
$field_options="<option>".__("Select Object",'cf7-pipedrive')."</option>"; 
  if(is_array($objects)){
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','cf7-pipedrive');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','cf7-pipedrive'));
  $valid_email=true;
...
$feed= $this->data->get_feed($fid); 
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','cf7-pipedrive'); 
  return;
  }
  //
