<?php
/**
* Plugin Name: Integration for Contact Form 7 and Pipedrive
* Description: Integrates Contact Form 7 and <a href="https://wordpress.org/plugins/contact-form-entries/">Contact Form Entries Plugin</a> with Pipedrive allowing form submissions to be automatically sent to your Pipedrive account 
* Version: 1.0.3
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Pipedrive Plugin','cf7-pipedrive') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','cf7-pipedrive') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Pipedrive Plugin','cf7-pipedrive') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
return $objects;
  ...
  */
  ...
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'cf7-pipedrive');
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', __('Create or update this entry in Pipedrive. The fields will be mapped according to the form feed settings.', 'cf7-pipedrive'), __('Send to Pipedrive', 'cf7-pipedrive'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.$this->id.'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Pipedrive. The fields will be mapped according to the form feed settings.','cf7-pipedrive').'" value="'.__('Send to Pipedrive', 'cf7-pipedrive').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Pipedrive Logs','cf7-pipedrive').'" href="'.$log_url.'">'.__('Pipedrive Logs','cf7-pipedrive').'</a>';
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','cf7-pipedrive')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
