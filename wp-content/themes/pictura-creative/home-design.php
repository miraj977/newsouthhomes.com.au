...
if (isset($_POST)) {
    ...
    if (isset($postlocation)) {
		if($postlocation == single-storey || $postlocation == double-storey){
			$tax = "storeys";
		}
		else{
			$tax = "home_types";
    ...
    $query = new WP_Query(
	 ...
	 if (is_page('split-level')) {
		$term = "split-level";?>
		 
<script>
jQuery(':input[data-filter="homestead"]').prop('disabled', true);
</script>

<style>
input[data-filter="homestead"]+span {
    cursor: not-allowed;
    background: #80808030;
}
    ...
    } elseif (is_page('duplex')) {
        $term = "duplex";
?>
		 
<script>
jQuery(':input[data-filter="homestead"]').prop('disabled', true);
jQuery(':input[data-filter="dual-occupancy"]').prop('disabled', true);
jQuery(':input[data-filter="granny-flats"]').prop('disabled', true);
</script>
...
input[data-filter="dual-occupancy"]+span {
    cursor: not-allowed;
    background: #80808030;
}input[data-filter="granny-flats"]+span {
    cursor: not-allowed;
    background: #80808030;
}
</style>
    ...
    if (!is_page('home-designs')) {
        if (is_page('split-level')) {$term = "split-level";}
        if (is_page('duplex')) {$term = "duplex";}
...
<div class="img-pad"
    ...
    $bedroom = $bedroom->name;
        if (!empty($bedroom)) {
			...
			if(get_sub_field('set_range')==1 && get_sub_field('bedroom_range')){
				echo get_sub_field('bedroom_range'). '</span>';
			}else{
    ...
    echo '<img class="bathIcon" src="' . get_template_directory_uri() . '/images/Bathtub@2x.svg" />';
			...
			if(get_sub_field('set_range')==1 && get_sub_field('bathroom_range')){
				echo get_sub_field('bathroom_range'). '</span>';
			}else{
